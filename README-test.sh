...
#Build the test container and install the current version of $(PACKAGE_NAME) from JFrog
# -d specifies the distro to test
TEST_MODE=true .github/packaging/common/test/entrypoint.sh -c -d el9
#...

#Execute the test runner
docker run -t -i $(PACKAGE_NAME)-pkg-tester-el9:$(git describe --tags --always)

#...
#test_execute.bats
# ✓ can run $(PACKAGE_NAME)
#
#1 test, 0 failures
